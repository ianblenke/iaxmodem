...
static int iax_xmit_frame(struct iax_frame *f)
{
	...
	/* Send the frame raw */
#ifdef DEBUG_SUPPORT
	if (debug && ntohs(h->scallno) & IAX_FLAG_FULL)
...
int iax_register(struct iax_session *session, char *server, char *peer, char *secret, int refresh, char *reason)
{
	...
	regreason = reason;
	if (regreason) {
	    iax_ie_append_str(&ied, IAX_IE_CAUSE, (unsigned char *) regreason);
...
#ifdef DEBUG_SUPPORT
	if (debug) iax_showframe(NULL, fh, 1, sin, datalen);
#endif
	...
	if (e) {
		...
		case AST_FRAME_IAX:
			...
			case IAX_COMMAND_POKE:
				send_command_final(session, AST_FRAME_IAX, IAX_COMMAND_PONG, ts, NULL, 0, -1);
				free(e);
				e = NULL;
				break;
						...
						int datalen)
{
	...
	if (e) {
		if (session->voiceformat > 0) {
			...
			ts = (session->last_ts & 0xFFFF0000L) | (ntohs(mh->ts) & 0xFFFF);
			ts = unwrap_timestamp(ts, session->last_ts);
			e->ts = ts;
