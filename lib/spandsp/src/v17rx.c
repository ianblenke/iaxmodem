 ...
 *
 * $Id: v17rx.c,v 1.82 2007/06/29 12:56:27 steveu Exp $
 */
...
enum
{
    ...
    TRAINING_STAGE_TRAIN_ON_CDBA_SLOWEQ,
...
static const complexf_t pulseshaper[PULSESHAPER_COEFF_SETS][V17_RX_FILTER_STEPS] =
{
    {
...
span_log(&s->logging, SPAN_LOG_FLOW, "Angles %x, %x, %x, %x, dist %d\n", s->angles[j], s->start_angles[0], s->angles[j | 0x1], s->start_angles[1], i);
        ...
        {
            /* This is probably a TEP signal.  So we park this modem and wait for the 
               real carrier to start after the TEP signal drops. */
            span_log(&s->logging, SPAN_LOG_FLOW, "Training failed (possible TEP)\n");
            /* Park this modem */
            s->agc_scaling_save = 0.0f;
            s->training_stage = TRAINING_STAGE_PARKED;
            s->put_bit(s->user_data, PUTBIT_TRAINING_FAILED);
    ...
    case TRAINING_STAGE_TRAIN_ON_CDBA:
        ...
        if (s->training_error < 1.0f || s->training_error > 200.0f || s->training_count == V17_TRAINING_SEG_2_LEN - 2000)
        {
            s->eq_delta *= EQUALIZER_SLOW_ADAPT_RATIO;
            s->carrier_track_i = 1000.0f;
            s->training_stage = TRAINING_STAGE_TRAIN_ON_CDBA_SLOWEQ;
        }
        break;
    case TRAINING_STAGE_TRAIN_ON_CDBA_SLOWEQ:
        /* Train on the scrambled CDBA section. */
        bit = descramble(s, 1);
        bit = (bit << 1) | descramble(s, 1);
        target = &cdba[bit];
    ...
    case TRAINING_STAGE_TRAIN_ON_CDBA_AND_TEST:
        ...
        /* By this point we should be sufficiently trained, and we don't really need the full-size
           of this training segment to know if the training is good or not.  A minor disturbance 
           in the audio during this comparatively lengthy portion could knock us off-balance.
           So we simply ignore the last 20 symbols for this reason and also because it seems that
           some signals don't carry-out this segment to a proper length, anyway. */
