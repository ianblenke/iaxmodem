 ...
 *
 * $Id: t38_gateway.c,v 1.84 2007/07/29 17:56:41 steveu Exp $
 */
            ...
            if (++t->num_bits == 8)
            {
                ...
                else
                {
                    ...
                    if (++t->len > 2)
                    {
                        ...
                        if (s->current_tx_data_type != T38_DATA_V21)
                        {
                            s->rx_data[s->rx_data_ptr] = bit_reverse8(t->buffer[next_out]);
                        }
                        else
                        {
                            if (s->corrupt_the_frame_to_t38)
                            {
                                /* Make sure the frame header goes out, before corrupting the rest */
                                s->rx_data[s->rx_data_ptr] = (next_out > 2)  ?  0  :  bit_reverse8(t->buffer[next_out]);
                            }
                            else
                            {
